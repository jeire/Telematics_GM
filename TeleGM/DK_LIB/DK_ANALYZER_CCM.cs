...
{   
    ...
    class DK_ANALYZER_CCM
    {   
        private const byte DEFRES_ETX  = 0x7E;
        private const string HEX_CHARS = "0123456789ABCDEF";

        
        private List<TBLDATA0> LstTBLccm = new List<TBLDATA0>();       //CCM 명령 테이블 리스트
        private DK_LOGGER tmpLogger = new DK_LOGGER("PC", false);

        public DK_ANALYZER_CCM()
        {
            tmpLogger.LoadTBL0("CCM.TBL", ref LstTBLccm);
        }
        ...
        public int AnalyzePacket(byte[] tmpBytes, ref string strCommand, ref string rtnData, byte[] strOrginSendData, string strCommandName, ref string strLogString)
        {
            ...
            {
                string strExMsg = "Exception:" + System.Reflection.MethodBase.GetCurrentMethod().Name + ":CCM:" + ex.Message;
                STEPMANAGER_VALUE.DebugView(strExMsg);
                tmpLogger.WriteCommLog("Exception: AnalyzePacket() CCM:", ex.Message, false);
        ...
        public PACKTYPE GetCcmResFormat(List<TBLDATA0> lstTBLDB, string strCommandName, ref bool bFind, ref int iDataIndex, ref int iLength, ref byte[] bytesOriginPack, ref int iIndexFormat)
        {
            PACKTYPE iResult = new PACKTYPE();
            iResult.iType = (int)CCMRESTYPE.NONE;
            iResult.iPos = -1;
            bFind = false;
            try
            {
                for (int i = 0; i < lstTBLDB.Count; i++)
                {
                    if (lstTBLDB[i].CMDNAME.Equals(strCommandName))
                    {
                        ...
                        {
                            case "SWVERSION":       iResult.iType = (int)CCMRESTYPE.SWVERSION;     
                                iDataIndex = 39;
                                iLength = 8;
                                iIndexFormat = 1;
                                break;
                            case "SWCOMFILEDATE":   iResult.iType = (int)CCMRESTYPE.SWCOMFILEDATE;
                                iDataIndex = 1;
                                iLength = 11;
                                iIndexFormat = 1;
                                break;
                            case "HWREVISION":      iResult.iType = (int)CCMRESTYPE.HWREVISION;     
                                iDataIndex = 4;
                                iLength = 4;
                                iIndexFormat = 4;
                                break;
                            case "MDMCHIPVER":      iResult.iType = (int)CCMRESTYPE.MDMCHIPVER;     
                                iDataIndex = 4;
                                iLength = 4;
                                iIndexFormat = 1;                                
                                break;
                            case "EXTCHAR": iResult.iType = (int)CCMRESTYPE.EXTCHAR;      
                                iDataIndex = 3;
                                iLength = 128; //EXT CHAR 는 전부 다읽는다.
                                iIndexFormat = 3;                                
                                break;
                            case "EXTINT32": iResult.iType = (int)CCMRESTYPE.EXTINT32;
                                iDataIndex = 3;
                                iLength = 4; 
                                iIndexFormat = 3;
                                break;
                            case "NV946": iResult.iType = (int)CCMRESTYPE.NV946;
                                iDataIndex = 3;
                                iLength = 3;
                                iIndexFormat = 3;
                                break;
                            case "EXTDEFAULT": iResult.iType = (int)CCMRESTYPE.EXTDEFAULT; 
                                iDataIndex = 3;
                                iLength = 128;  //EXTDEFAULT 는 전부 다읽는다.
                                iIndexFormat = 135;
                                break;
                            case "STDCHAR":         iResult.iType = (int)CCMRESTYPE.STDCHAR;
                                iDataIndex = 4;
                                iLength = 1;  //STDCHAR 도 전부 다읽기는 하지만 전체사이즈를 모르기때문에 1로 설정해두고 전부 다읽는다.
                                iIndexFormat = 4;
                                break;
                            case "QFUSE_NORMAL":    iResult.iType = (int)CCMRESTYPE.QFUSE_NORMAL;   
                                iDataIndex = 5;
                                iLength = 1;  //STDCHAR 도 전부 다읽기는 하지만 전체사이즈를 모르기때문에 1로 설정해두고 전부 다읽는다.
                                iIndexFormat = 3;                                
                                break;                            
                            case "DTC_RF":          iResult.iType = (int)CCMRESTYPE.DTC_RF;     
                                iDataIndex = 1;
                                iLength = 1;  
                                iIndexFormat = 1;                                
                                break;
                            case "DTC_GPS":         iResult.iType = (int)CCMRESTYPE.DTC_GPS; 
                                iDataIndex = 2;
                                iLength = 1;
                                iIndexFormat = 1;
                                break;
                            case "STDDEFAULT":       iResult.iType = (int)CCMRESTYPE.STDDEFAULT;
                                iDataIndex = 0;
                                iLength = 1;  //STDCHAR 도 전부 다읽기는 하지만 전체사이즈를 모르기때문에 1로 설정해두고 전부 다읽는다.
                                iIndexFormat = 4;
                                break;

                            case "GPS_NORMAL": iResult.iType = (int)CCMRESTYPE.GPS_NORMAL;
                                iDataIndex = 2;
                                iLength = 1;
                                iIndexFormat = 4;
                                break;

                            case "GPS_INFO": iResult.iType = (int)CCMRESTYPE.GPS_INFO;
                                iDataIndex = 2;
                                iLength = 1;
                                iIndexFormat = 2;
                                break;

                            case "MODECHANGE": iResult.iType = (int)CCMRESTYPE.MODECHANGE;
                                iDataIndex = 0;
                                iLength = 1;  
                                iIndexFormat = 7;
                                break;

                            case "NORESPONSE": iResult.iType = (int)CCMRESTYPE.NORESPONSE; break;
                            default:                iResult.iType = (int)CCMRESTYPE.NONE;           break;
                        }
            ...
            {
                tmpLogger.WriteCommLog("Exception: GetCcmResFormat :", ex.Message, false);
        ...
        public string GetCcmCmdType(List<TBLDATA0> lstTBLDB, string strCommandName)
        {
            try
            {
                for (int i = 0; i < lstTBLDB.Count; i++)
            ...
            {
                tmpLogger.WriteCommLog("Exception: GetCcmCmdType :", ex.Message, false);
                string strExMsg = "Exception:" + System.Reflection.MethodBase.GetCurrentMethod().Name + ":" + ex.Message;
                STEPMANAGER_VALUE.DebugView(strExMsg);
        ...
        {            
            ...
            {                   
                ...
                int iIndexFormat = 0;
                PACKTYPE ccmp = GetCcmResFormat(LstTBLccm, strCommandName, ref bCcmpRes, ref iDataIndex, ref iDataLength, ref bytesOriginFormat, ref iIndexFormat);
                ...
                if (bCcmpRes)
                {
                    ...
                    switch (ccmp.iType)
                    {
                        case (int)CCMRESTYPE.NORESPONSE: //NORESPONSE 형태는 응답이 없다. 물론 여기까지 탈일은 없다... 그러므로 절차서에서 SEND 옵션으로해야한다.
                                                    rtnData = ""; return (int)STATUS.OK;

                        case (int)CCMRESTYPE.STDDEFAULT: //STDDEFAULT 형태는 응답이 보낸것와 완전히 일치한다. 
                        ...
                        case (int)CCMRESTYPE.EXTDEFAULT: //EXTDEFAULT 형태는 응답이 보낸것와 완전히 일치한다. 
                    ...
                    switch (ccmp.iType)
                    {
                        case (int)CCMRESTYPE.SWVERSION:
                        case (int)CCMRESTYPE.SWCOMFILEDATE:
                                                    rtnData = Encoding.UTF8.GetString(tmpDecode, iDataIndex, iDataLength);
                                                    break;
                        case (int)CCMRESTYPE.HWREVISION:
                                                    ...
                                                    break;
                        case (int)CCMRESTYPE.MODECHANGE:
        ...
        public byte[] ConvertCcmByteHexString(string strPacket, ref string strSendPack, string strParam, ref bool brtnOk, ref string strReason, string strCommandName)
        { //strSendPack은 TX에서 로깅하기 위해사용한다. rx에서는 따로 해주어야한다.
            ...
            string strPackType = GetCcmCmdType(LstTBLccm, strCommandName);
