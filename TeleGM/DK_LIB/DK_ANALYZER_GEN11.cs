...
{   
    ...
    class DK_ANALYZER_GEN11
    {        
        private const byte DEFRES_STX1 = 0x02;
        private const byte DEFRES_STX2 = 0xFB;
        private const byte DEFRES_ETX  = 0xFA;

        /*private int ALDLMAX = 200;  //GEN10 TCP 는 150 , GEN11 은 200 이라고 함. by 강종훈책임 20170607 메일
                                    //GEN11 2018년 7월 6일 이후로 사이즈가 250으로 다시 증가한다고함. by 강종훈 책임 20180706 메일
                                    //이에 따라 절차서 명령으로 ALDLMAX 사이즈를 변경할수 있도록 하자.*/
        private const string HEX_CHARS = "0123456789ABCDEF";

        private List<TBLDATA0> LstTBLGen11 = new List<TBLDATA0>();       //GEN10 명령 테이블 리스트
        private DK_LOGGER tmpLogger = new DK_LOGGER("PC", false);

        public DK_ANALYZER_GEN11()
        {            
            tmpLogger.LoadTBL0("GEN11.TBL", ref LstTBLGen11);
        }
                ...
                for (int i = 0; i < lstTBLDB.Count; i++)
                {
                    ...
                    {                        
                        switch (lstTBLDB[i].RECVPAC)
                        {
                            ...
                            case "DTC_TABLE_GB":iResult.iType = (int)GEN11RESTYPE.DTCGB;    break; //GEN11 GB모델 DTC
                            case "DTC_TABLE_GEM":iResult.iType = (int)GEN11RESTYPE.DTCGEM;   break; //GEN11 GEM모델 DTC
